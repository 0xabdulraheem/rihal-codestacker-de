...
**Quarantine vs. silent discard for bad data.** Records with negative costs, null customer IDs, cancelled statuses, and duplicates are filtered out during transformation. Rather than silently dropping them, I implemented a `staging.quarantine` table that captures every rejected record as JSONB alongside a categorical rejection reason. This adds a small write overhead per run but provides full auditability: data stewards can inspect exactly what was rejected and why via the Quarantine tab in the dashboard. The quarantine is idempotent (cleared and repopulated each run) so it does not grow unboundedly.

---

## Where could your solution still fail?

**API schema changes.** The `validate_api_response()` function validates every record in the batch for required field presence and catches structural changes (e.g., renaming `data` to `results`). However, it checks for field existence, not type correctness. If the API starts returning `shipping_cost` as a string instead of a number, the validation passes but the database INSERT may fail or produce unexpected type coercion.
...
**Batch INSERT via `execute_values()`** handles the current scale well, but at 200,000 rows the Python-side tuple construction and network transfer become measurable. I would switch to PostgreSQL's `COPY` protocol for bulk loading, which streams data directly to the database without constructing an intermediate SQL string.
...
**Data quality framework expansion.** The current `validate_data_quality` step runs ten checks and logs results to `analytics.data_quality_log`, and the quarantine table captures rejected records with categorical reasons. With more time, I would make checks configurable via YAML (threshold-based alerts, custom SQL checks), add Slack/email alerting when checks fail or quarantine counts exceed a threshold, and build a historical trend view showing rejection rates over time to detect gradual upstream degradation.
...
```
Opens a database connection and cursor using context managers from `db.py`. psycopg2 implicitly begins a transaction on the first SQL statement, so all following operations are atomic. If anything fails before `conn.commit()`, the context manager's exception handler calls `conn.rollback()`.
...
INSERT INTO analytics.shipping_spend_by_tier
    (tier, year_month, total_shipping_spend, shipment_count, avg_shipping_cost)
SELECT
    tier,
    TO_CHAR(shipment_date, 'YYYY-MM') AS year_month,
    SUM(shipping_cost)                 AS total_shipping_spend,
    COUNT(*)                           AS shipment_count,
    ROUND(SUM(shipping_cost) / COUNT(*), 2) AS avg_shipping_cost
