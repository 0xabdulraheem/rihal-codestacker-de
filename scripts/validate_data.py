...
def _check(failures, name, passed, details, cur=None):
    record_quality_check(name, passed, details, cur=cur)
    if not passed:
        ...
        with get_connection() as conn:
            with get_cursor(conn) as cur:
                ...
                _check(failures, "raw_not_empty", raw_count > 0,
                       f"raw.shipments has {raw_count} rows", cur=cur)

                _check(failures, "deduped_less_than_raw", deduped_count <= raw_count,
                       f"deduped={deduped_count} raw={raw_count}", cur=cur)

                _check(failures, "enriched_matches_deduped", enriched_count == deduped_count,
                       f"enriched={enriched_count} deduped={deduped_count}", cur=cur)

                _check(failures, "analytics_not_empty", analytics_count > 0,
                       f"analytics has {analytics_count} tier-month combinations", cur=cur)
                ...
                _check(failures, "no_negative_costs_in_staging", neg_count == 0,
                       f"found {neg_count} non-positive costs", cur=cur)
                ...
                _check(failures, "no_duplicate_shipment_ids", dup_count == 0,
                       f"found {dup_count} duplicate shipment IDs", cur=cur)
                ...
                _check(failures, "no_null_tiers_in_enriched", null_tier_count == 0,
                       f"found {null_tier_count} null tiers", cur=cur)
                ...
                _check(failures, "no_non_positive_spend_in_analytics", neg_spend == 0,
                       f"found {neg_spend} non-positive spend rows", cur=cur)
                ...
                _check(failures, "analytics_total_matches_enriched", totals_match,
                       f"analytics_sum={analytics_total} enriched_sum={enriched_total}", cur=cur)

                quarantine_count = _count(
                    cur, "SELECT COUNT(*) FROM staging.quarantine"
                )
                _check(failures, "quarantine_records_logged", True,
                       f"{quarantine_count} rejected records quarantined for review", cur=cur)

                conn.commit()

        total_checks = 10
        stage_metrics["rows_processed"] = total_checks - len(failures)
        stage_metrics["rows_rejected"] = len(failures)

    if failures:
        raise ValueError(f"Data quality checks failed: {failures}")

    logger.info("All %d data quality checks passed", total_checks)
